...
public class DSPEvent implements Listener {
    ...
    public void onChat(final PlayerChatEvent e) {
        ...
        if (DSPFunction.currentEditPrefix.containsKey(p.getUniqueId())) {
            ...
            this.plugin.getConfig().set("Settings.PrefixList." + name, prefix);
            plugin.saveDataContainer();
            p.sendMessage(this.plugin.prefix + name + plugin.getLang().get("prefix_display_set") + ColorUtils.applyColor(prefix));
        ...
        this.plugin.getConfig().getConfigurationSection("Settings.PrefixList").getKeys(false).forEach(s -> {
            if (s.equals(name)) {
                e.setFormat(ColorUtils.applyColor(this.plugin.getConfig().getString("Settings.PrefixList." + name)) + e.getFormat());
            }
        });
    }
